...
| `playbook.boot_pmon_stuck` | playbook | board_system | book_reference | needs_board_check |
| `playbook.boot_kernel_panic` | playbook | board_system | book_reference | needs_board_check |
...
| `playbook.camera_opencv_failure` | peripheral | playbook |
...
| `playbook.qt_deploy_failure` | project | playbook |
| `playbook.opencv_build_failure` | project | playbook |
| `playbook.openwrt_port_failure` | project | playbook |
| `playbook.dpdk_init_failure` | project | playbook |

说明：

- 外设接线、GPIO 写操作、SPI 传输和未知 bus 扫描必须保持保守。
- 任何涉及写外设状态的内容必须写入“禁止操作”或“需人工确认”。
- DPDK、OpenWrt、OpenCV 编译成本高，不作为第一批 MVP。

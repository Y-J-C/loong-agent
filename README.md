...
- Provider 支持 OpenAI-compatible `/chat/completions`、profile、capabilities、thinkingLevel 提示和 usage 审计占位，默认尝试真实 SSE streaming，不支持时自动 fallback。
- RPC / SDK 集成层已落地，可通过 `node src/index.js rpc` 使用 stdin/stdout JSONL 驱动单会话 Agent Runtime，也可通过 `src/sdk.js` 以 child process 方式嵌入。
- Release pack 可在真实 LS2K1000 PAI UDB v1.5 / Node 14.16.1 板端解压即运行。

## 当前状态

Loong-Agent 当前已有八个阶段的工程收口，并通过测试、源码和契约文档形成可追溯证据：
...
| 8. RPC / SDK 集成层 | 已完成 | stdin/stdout JSONL RPC、child process SDK、单会话驱动、真实 provider abort。 |
...
| OpenAI-compatible Provider、profile、capability、streaming、usage | provider profile、capability、usage、fallback、abort、coalescing、TUI partial JSON 测试 | `src/provider-registry.js`、`src/llm.js`、`src/agent-loop.js`、`docs/provider-streaming-contract.md` | `node scripts/test-streaming.js`、`node scripts/test-runtime.js` |
| RPC / SDK 单会话集成、JSONL 协议、真实 abort | RPC ready/prompt/status/busy/steer/followUp/stream abort/non-stream abort/SDK smoke 测试 | `src/rpc.js`、`src/sdk.js`、`src/index.js`、`scripts/test-rpc.js` | `node scripts/test-rpc.js` |
...
- `LOONG_AGENT_PROVIDER_PROFILE=deepseek` 为默认 profile；`ollama` 使用 `http://127.0.0.1:11434/v1` 和 `llama3.1`；`custom` 使用显式 provider/baseUrl/model 配置。
- `LOONG_AGENT_THINKING_LEVEL=off|low|medium|high` 默认只控制分析深度提示；DeepSeek `deepseek-v4-pro` / `deepseek-v4-flash` 会使用官方 native thinking 参数，session 只记录 native thinking 元数据，不保存思维链正文。
...
RPC 模式使用 stdin/stdout JSONL。stdout 只输出 JSONL 事件，诊断和错误走 stderr 或 `rpc_error` 事件。第一版只维护一个长生命周期 Agent session，不支持多并发 session。
...
SDK 使用 child process 驱动 `node src/index.js rpc`，提供：
...
每次成功模型调用后会写入 `model_usage` 事件，记录 provider、profile、model、capabilities、thinkingLevel、streaming/fallback 状态，以及 token usage。Provider 未返回 token 时标记为 `not_reported` 和「待确认」；`agent_end.usageSummary` 会汇总本次 run 的模型调用用量。

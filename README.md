...
After starting a background command, verify it with `process_status`, inspect logs with `process_logs`, and read generated files such as CSV output. Foreground timeout returns `exitCode=124`, `timedOut=true`, and a recovery hint instead of being treated as a model crash.

...
| Current USB camera V4L2 path is unavailable because `/dev/video*` and `uvcvideo.ko` are absent and media support is disabled | `usb_camera_uvc_boundary` | `kb/usb_camera_uvc_boundary.md`, `kb/facts/camera_opencv.json`, `kb/playbooks/usb-camera-no-dev-video.md` | high |

...
- Playbooks: `kb/playbooks/usb-camera-no-dev-video.md`, `kb/playbooks/usb-camera-userland-uvc-capture.md`, `kb/playbooks/opencv-numpy-conflict.md`, `kb/playbooks/opencv-haar-face-detection.md`

用户目录 NumPy `1.19.5` 触发 `undefined symbol: zungqr_` 当前未复现，作为 historical 已知故障保留；`libusb + libuvc` 抓帧流程来自资料，尚未重新跑抓帧验证，仍保持 `external_reference + needs_board_check`。

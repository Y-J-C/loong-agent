...
function isEditorSlotOccupied(state) {
  return Boolean(state && (state.pendingToolApproval || state.selector || state.activePanel || state.settingsMenu || state.modelSelector));
}

function getFocusedSurface(state) {
  if (state && state.pendingToolApproval) return { id: 'approval', occupied: true };
